package cn.ling.util.telegram;

import cn.ling.config.properties.BlogProperties;
import cn.ling.config.properties.TelegramProperties;
...
public class TelegramBotMsgHandler {
	...
	private String replyComment(String msgContent) {
		...
		if (splitIndex2 != -1 && msgContent.length() > splitIndex2 + 1) {
			...
			commentUtils.setAdminCommentByTelegramAction(commentDTO);
			...
			return String.format(
					...
					commentDTO.getEmail(),
					commentDTO.getPublished() ? "公开" : "待审核",
